...
{
	// Initialize forces array with zero Vec3, force[i] will later be the sum of all forces acting on particle i
    let forces = new Array(positions.length);
    for (let i = 0; i < forces.length; i++) {
        forces[i] = new Vec3(0, 0, 0);
    }

    // Add downward gravitational force F = m * g
    for (let i = 0; i < forces.length; i++) {
        forces[i].inc(gravity.mul(particleMass));
    }

    // Compute spring forces (Hooke’s law) and damping (to reduce oscillation)
    for (const s of springs) {
        ...
        if (dist === 0) continue;  // we skip computation to avoid division by zero

        const dir = deltaP.div(dist);
        const deltaV = v_j.sub(v_i);

        const Fs = dir.mul(stiffness * (dist - s.rest));  // Hooke's law
        const Fd = dir.mul(damping * deltaV.dot(dir));  // damping along spring direction
        const F = Fs.add(Fd);
    ...
    // Euler integration for positions and velocities
    for (let i = 0; i < positions.length; i++) {
        const acc = forces[i].div(particleMass);  // F = m * a -> a = F / m
        velocities[i].inc(acc.mul(dt));  // v = v + a * dt
        positions[i].inc(velocities[i].mul(dt));   // x = x + v * dt
    }

    // Collision with bounding box [-1, 1] in each axis
    for (let i = 0; i < positions.length; i++) {
            if (positions[i][dim] < -1) {  // if outside the boundaries (i.e. walls hit)
                positions[i][dim] = -1;  // bounce off
                if (velocities[i][dim] < 0) 
					velocities[i][dim] *= -restitution;  // loss of velocity
            }
            if (positions[i][dim] > 1) {  // if outside the boundaries (i.e. walls hit)
                positions[i][dim] = 1;  // bounce off
                if (velocities[i][dim] > 0) 
